 ...
 *      }
 *      // Output: <3>[MQTT-Client] mqtt_connect_fn(): ERROR: Broker connection refused. RC: -1
 *
 ...
 *      }
 *      // Output: <3>[MQTT-Client] open_fifo(): SYS_ERR: Failed to open FIFO: No such file or directory
 *
 ...
 *      // Output: [MQTT-Client] DBG (client.c:42): Packet payload size: 256 bytes
 *
 * ========================================================================== */
...
#define LOG_WARN(fmt, ...) \
    fprintf(stderr, LOG_LVL_WARNING "[%s] WARN: " fmt "\n", MODULE_NAME, ##__VA_ARGS__)#define LOG_INFO(fmt, ...) \
...
#define LOG_SYS_ERROR(fmt, ...) \
    fprintf(stderr, LOG_LVL_ERR "[%s] SYS_ERR: " fmt ": %s\n", \
            MODULE_NAME, ##__VA_ARGS__, strerror(errno))
