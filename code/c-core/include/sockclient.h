...
int ipc_client_connect(const char* socket_path){
    ...
    while (retries > 0) {
        if (connect(client_fd, (struct sockaddr *)&addr, sizeof(struct sockaddr_un)) == 0){
            LOG_INFO("Connected successfully to server at %s. fd: %d", socket_path, client_fd);
            return client_fd;
        }
    ...
    LOG_ERROR("Failed to connect to server at %s after multiple attempts: %s", socket_path, strerror(errno));
    close(client_fd);
    return -1;
}
...
int ipc_client_send(int fd, const IPCMessage* msg){
    ...
    if (bytes_sent == -1) {
        if(errno == EPIPE || errno == ECONNRESET){
            LOG_ERROR("Connection to server lost while sending message: %s", strerror(errno));
        } else {
    ...
    LOG_DEBUG("Successfully sent message to server. Origin: %d, Type: %d", msg->origin, msg->msgtype);
...
int ipc_client_receive(int fd, IPCMessage* msg){
    ...
    LOG_DEBUG("Successfully received message from server. Origin: %d, Type: %d", msg->origin, msg->msgtype);
