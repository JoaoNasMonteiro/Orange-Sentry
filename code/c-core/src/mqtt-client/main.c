...
#define PUB_FIFO_NAME "/tmp/mqtt_pub_fifo"
#define cmd_FIFO_NAME "/tmp/mqtt_cmd_fifo"
 ...
 * */
 ...
 int main() {
  ...
  //init Pub and command (sub) channels 
  IPC_Channel pub_channel = {0};
  pub_channel.fd = -1;
  if (ipc_open_channel(&pub_channel, PUB_FIFO_NAME) != 0) {
    LOG_ERROR("Failed to open pub FIFO at %s", PUB_FIFO_NAME);
    return -1;
  }

  IPC_Channel cmd_channel = {0};
  cmd_channel.fd = -1;
  if (ipc_open_channel(&cmd_channel, cmd_FIFO_NAME) != 0) {
    LOG_ERROR("Failed to open command FIFO at %s", cmd_FIFO_NAME);
    return -1;
  }

  // MQTT client initialization
  LOG_DEBUG("Starting MQTT Client");
  mqttContext *ctx = mqtt_create(ADDRESS, CLIENTID, 20, &arena, &cmd_channel, &pub_channel);
  if (ctx == NULL) {
  ...
  while (keepRunning) {
    ...
    // Implementar lógica de loop central com polling dos pipes de IPC para evitar busy waiting 
